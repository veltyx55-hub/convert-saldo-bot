...
TESTIMONI_GROUP_ID_INT = int(config.TESTIMONI_GROUP_ID)
...
# ID khusus Telegram untuk admin yang posting secara anonim ("Remain Anonymous").
# Hanya admin nyata yang memiliki izin ini yang bisa posting sebagai GroupAnonymousBot.
...
def mask_phone_number(number: str) -> str:
    """Sensor nomor telepon: 083873 → 083******73, 08123456789 → 081******89."""
    if not number:
        return "-"
    n = number.strip()
    if len(n) <= 4:
...
def join_group_keyboard() -> InlineKeyboardMarkup:
    keyboard = []
    if config.TESTIMONI_GROUP_INVITE_LINK:
        keyboard.append(
            [InlineKeyboardButton("\U0001F517 Gabung Grup Testimoni", url=config.TESTIMONI_GROUP_INVITE_LINK)]
        )
    keyboard.append(
...
async def is_group_admin(context: ContextTypes.DEFAULT_TYPE, chat_id: int, user_id: int) -> bool:
    # GroupAnonymousBot (ID 1087968824) mewakili admin yang mengaktifkan "Tetap Anonim".
    # Hanya admin nyata di grup yang bisa posting sebagai anonymous.
    # Validasi tambahan: hanya izinkan dari ORDER_GROUP_ID supaya tidak bocor ke chat lain.
    if user_id == ANONYMOUS_ADMIN_USER_ID:
        if chat_id == ORDER_GROUP_ID_INT:
            logger.info(
                "Admin check: GroupAnonymousBot (%s) di ORDER_GROUP (%s) — diizinkan sebagai anonymous admin.",
                user_id, chat_id,
            )
            return True
        logger.warning(
            "Admin check: GroupAnonymousBot (%s) di chat %s (bukan ORDER_GROUP) — ditolak.",
            user_id, chat_id,
        )
        return False
    ...
    try:
        ...
        if is_admin:
            logger.info(
                "Admin check: user %s (@%s) di chat %s status='%s' — diizinkan.",
                user_id,
                getattr(member.user, "username", "?"),
                chat_id,
                member.status,
            )
        else:
            logger.warning(
                "Admin check GAGAL: user %s (@%s) di chat %s status='%s' — bukan admin/owner.",
                user_id,
                getattr(member.user, "username", "?"),
                chat_id,
                member.status,
        ...
        logger.error(
            "Admin check ERROR: gagal memanggil getChatMember untuk user %s di chat %s.",
            user_id, chat_id,
        )
        traceback.print_exc()
        return False
...
async def send_join_group_gate(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    text = (
        ...
        "Untuk menggunakan bot ini, kamu wajib bergabung ke *Grup Testimoni* kami terlebih dahulu.\n\n"
        "Klik *Gabung Grup Testimoni*, lalu klik *Verifikasi* setelah kamu bergabung."
    )
...
async def menu_riwayat(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for order in orders:
        ...
        lines.append(
            ...
            f"\U0001F522 Nomor Tujuan: {mask_phone_number(order['nomor_tujuan'] or '')}\n"
...
async def admin_receive_completion_proof(
    ...
    await context.bot.send_photo(
        ...
        caption=(
            ...
            f"Nominal: Rp{format_rupiah(order['nominal'])}\n"
            f"Tujuan Saldo: {order['ewallet']}\n"
            f"Nomor Tujuan: {mask_phone_number(order['nomor_tujuan'] or '')}\n\n"
    ...
    # Kirim notifikasi teks ke grup testimoni (nomor disensor, tanpa foto)
    try:
        await context.bot.send_message(
            chat_id=TESTIMONI_GROUP_ID_INT,
            text=(
                "\U0001F389 *Order Selesai*\n\n"
                f"\U0001F9FE Order ID: `{order_id}`\n"
                f"\U0001F4B0 Nominal: Rp{format_rupiah(order['nominal'])}\n"
                f"\U0001F3E6 Tujuan Saldo: {order['ewallet']}\n"
                f"\U0001F4F1 Nomor Tujuan: {mask_phone_number(order['nomor_tujuan'] or '')}\n\n"
                "\u2705 Status: Selesai\n\n"
                "Terima kasih sudah menggunakan CV Payment System."
            ),
            parse_mode=ParseMode.MARKDOWN,
        )
        logger.info("Notifikasi testimoni order %s berhasil dikirim ke grup testimoni.", order_id)
    except Exception:
        logger.error("Gagal mengirim notifikasi ke grup testimoni untuk order %s.", order_id)
        traceback.print_exc()
